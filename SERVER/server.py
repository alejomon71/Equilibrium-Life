...
def uploadVideo():
    ...
    file.save("C:/Users/Jeronimo/Desktop/PROYECTO FINAL/EQUILIBRU-IUM/Equilibrium Life/myproject/SERVER/static/videos/"+secure_filename(file.filename))
    conexion=MySQLdb.connect("localhost", "root", "admin", "proyecto")
    cursor=conexion.cursor()
...
def uploadBook():
    file=request.files["infoFile"]
    file.save("C:/Users/Jeronimo/Desktop/PROYECTO FINAL/EQUILIBRU-IUM/Equilibrium Life/myproject/SERVER/static/libros/"+secure_filename(file.filename))
    conexion=MySQLdb.connect("localhost", "root", "admin", "proyecto")
    cursor=conexion.cursor()
    cursor.execute(f"INSERT INTO libros VALUES ('{secure_filename(file.filename)}',{int(request.form["estado"])},{int(request.form["tipo"])})")
    conexion.commit()
    enviarEmail(getAllEmails(),request.form["subject"], "<h3 style:'font-family:Courier'>"+request.form["message"]+"</h3>")
    return "uploaded"
...
def setP_L():
    ...
    cursor.execute(f"INSERT INTO playlist VALUES('{int(data.get("genre"))}', '{data.get("url")}')")
